...
export class RecipeService {
  ...
  private endpoint3: string = 'http://www.tastyboutique.tk:5341/api/v1/search';
  ...
  postCollections(recipes:CollectionsModel): Observable<any> {
    return this.http.post<any>(`${this.endpoint2}`, recipes, this.httpOptions);
  }
  getAllCollections(): Observable<RecipesGetModel[]> {
    return this.http.get<RecipesGetModel[]>(`${this.endpoint2}`, this.httpOptions);
  }
  ...
  getSearch(recipeId: string): Observable<RecipesGetModel> {
    return this.http.get<RecipesGetModel>(`${this.endpoint3}`, this.httpOptions);
  }
